...
mod tests {
    ...
    #[test]
    fn test_1790() {
        let s1 = "bank".to_owned();
        let s2 = "kanb".to_owned();
        let expected = true;
        assert_eq!(Solution::are_almost_equal(s1, s2), expected);
        let s1 = "attack".to_owned();
        let s2 = "defend".to_owned();
        let expected = false;
        assert_eq!(Solution::are_almost_equal(s1, s2), expected);
        let s1 = "kelb".to_owned();
        let s2 = "kelb".to_owned();
        let expected = true;
        assert_eq!(Solution::are_almost_equal(s1, s2), expected);
    }
}
