...
// discuss: https://leetcode.com/problems/fruits-into-baskets-ii/discuss/?currentPage=1&orderBy=most_votes&query=
...
impl Solution {
    pub fn num_of_unplaced_fruits(fruits: Vec<i32>, baskets: Vec<i32>) -> i32 {
        ...
        let mut placed = vec![false; fruits.len()];

        for fruit in fruits {
            for (i, &basket) in baskets.iter().enumerate() {
                if !placed[i] && fruit <= basket {
                    placed[i] = true;
                    result -= 1;
                    break;
                }
