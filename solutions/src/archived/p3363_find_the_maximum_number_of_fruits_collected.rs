...
// discuss: https://leetcode.com/problems/find-the-maximum-number-of-fruits-collected/discuss/?currentPage=1&orderBy=most_votes&query=
...
impl Solution {
    pub fn max_collected_fruits(fruits: Vec<Vec<i32>>) -> i32 {
        ...
        for i in 0..n - 2 {
            for j in (n - 1 - i)..n {
                for next_j in (j - 1).max(i + 2)..=(j + 1).min(n - 1) {
                    next[next_j] = next[next_j].max(cur[j] + fruits[i + 1][next_j]);
                }
            }
        ...
        for j in 0..n - 2 {
            for i in (n - 1 - j)..n {
                for next_i in (i - 1).max(j + 2)..=(i + 1).min(n - 1) {
                    next[next_i] = next[next_i].max(cur[i] + fruits[next_i][j + 1]);
                }
            }
