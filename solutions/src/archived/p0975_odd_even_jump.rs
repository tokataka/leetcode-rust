...
impl Solution {
    pub fn odd_even_jumps(arr: Vec<i32>) -> i32 {
        ...
        let mut val_idx_map = BTreeMap::new();

        for (i, &val) in arr.iter().enumerate().rev() {
            if let Some((_, &odd_idx)) = val_idx_map.range(val..).next() {
                odd[i] = even[odd_idx];
            }

            if let Some((_, &even_idx)) = val_idx_map.range(..=val).next_back() {
                even[i] = odd[even_idx];
            }

            val_idx_map.insert(val, i);
        }
...
mod tests {
    ...
    fn test_975() {
        ...
        let expected = 2;
        assert_eq!(Solution::odd_even_jumps(arr), expected);
